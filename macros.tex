...
\setecfont{chapfont}{cmssdc10}{\yuan}{40}
\setecfont{sectfont}{cmbx14}{\hei}{14}
%
...
\def\leftheadline{\hbox to \pagewidth{%
    ...
    {\tenbf\folio}\hskip10pt\rbook\hfil% folio to left of text
    %\rhead% running head flush left
    }}
\def\rightheadline{\hbox to \pagewidth{%
    \vbox to 10pt{}% strut to position the baseline
    %\rhead% running head flush right
    \hfil\rhead\hskip10pt {\tenbf\folio}% folio to right of text
    }}
...
%    \advance\pageno by-1 \fi
  ...
  {\def\TeX{T\kern-.2em\lower.5ex\hbox{E}\kern-.06em X}
    ...
    \def\\{#3}
    \ifx\empty\\ \centerline{\chapfont 第#2章}%
    \else\centerline{\chapfont 第#2\kern-.06em#3章}\fi%
    \vskip 1pc
    \baselineskip 20pt \lineskiplimit \titlelsl \lineskip 10pt
...
\outer\def\subsectnon#1 \par{\bigbreak
  \bookmark{2}{#1}
  \noindent{\sectfont #1\enspace}\smallbreak}

\outer\def\subsectcnon#1 \par{\bigbreak
  \bookmark{2}{#1}
  \centerline{\sectfont #1\enspace}\bigbreak}
